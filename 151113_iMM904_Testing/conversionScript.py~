...
if args_s[-4:] == '.xml':
	cobra_model = read_sbml_model('data/models/%s' % args_s)
if args_s[-4:] == '.mat':
	cobra_model = cobra.io.mat.load_matlab_model('data/models/%s' % args_s)
idRs = []
lb = []
ub = []
pathway = []
...
if args.l is not None:
	args_l = str(args.l)
	lb_file = open('data/models/%s' % args_l)
...
if args.u is not None:
	args_u = str(args.u)
	ub_file = open('data/models/%s' % args_u)
...
if args.g is not None:
	args_g = str(args.g)
	genes_genes2rxn_file = open('data/models/%s' % args_g)
