...
#    exportPickle(mDict, fOutModel % (description, str(repetition) + '_dict'))
...
@profile
def findActiveRxns_name(name, thresh, rl = []):
    cbm = importPickle(name)
    act = set()
    arrayIdRs = array(cbm.idRs[:])
    init = cbm.initLp()
    init 
...
def pruneRxn(cbm, cH, rxn, thresh, description, repetition, biomassRxn,
    ...
    try:
        ...
        name = '../data/sample_model.pkl'
        exportPickle(m0, name)
        importPickle('../data/sample_model.pkl')
        #NOTE the threshold for is set a bit higher for cH rxns
        act = findActiveRxns_name(name, thresh, cH)
        cH_act = cH & act
    ...
    try:
        ...
        act2 = findActiveRxns_name(name, thresh, cH)
        cH_act2 = cH & act2
