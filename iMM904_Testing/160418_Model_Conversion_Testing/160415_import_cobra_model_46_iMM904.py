...
print dir(cobra_model.genes[1])
genes_test = _cell(cobra_model.genes.list_attr("notes"))
print genes_test
grRules = _cell(cobra_model.reactions.list_attr("gene_reaction_rule"))
print grRules
grRules_test = _cell(cobra_model.reactions.list_attr("gene_name_reaction_rule"))
print grRules_test
#Rules = _cell(cobra_model.reactions.list_attr("rule"))
print dir(cobra_model.reactions[1])
...
model = {test_model[:-4] : {'rxns': rxns_matlab, 'mets': mets_matlab, 'ub': ub_matlab, 'lb': lb_matlab, 'S': S, 'grRules': grRules, 'genes': genes_matlab, 'rxnGeneMat': rxnGeneMat, 'rev': rev_cobra, 'c': c, 'subsystem': subsystem, 'metNames': metNames, 'metFormulas': metFormulas, 'b': b, 'description': test_model[:-4]}}
#Export the model as a .mat file for COBRA MATLAB usage
scipy.io.savemat('%s.mat' % test_model[:-4], model)
