...
description = str(cobra_model.id)
...
for i in idRs:
	...
	if i not in rxns_to_remove_2:
		...
		grRules = np.delete(grRules, rxn_index-1)
		c = np.delete(c, rxn_index-1)
		subsystem = np.delete(subsystem, rxn_index-1)
...
for i in idSp:
	...
	if i in idSp_to_remove:
		idSp.remove(i)
		metNames = np.delete(metNames, met_index-1)
		metForumals = np.delete(metFormulas, met_index-1)
		b = np.delete(b, met_index-1)
...
model = {'model' : {'rxns': rxns_matlab, 'mets': mets_matlab, 'ub': ub_matlab, 'lb': lb_matlab, 'S': S, 'grRules': grRules, 'genes': genes_matlab, 'rxnGeneMat': rxnGeneMat, 'rev': rev_cobra, 'c': c, 'subsystem': subsystem, 'metNames': metNames, 'metFormulas': metFormulas, 'b': b}}
