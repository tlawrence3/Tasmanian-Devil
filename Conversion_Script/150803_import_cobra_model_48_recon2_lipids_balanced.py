...
if args.c is not None:
	...
	unbalanced_rxns_removed = []
	for i in unbalanced_rxns:
		ARTF = re.search('ARTF', i)
		if ARTF is not None:
			unbalanced_rxns_removed.append(i)
			for j in rxns[i]['reactants']:
				reactant_stoich = int(metabolite_dict[j])*rxns[i]['reactants'][j]
			product_stoich = 37
			new_stoich = reactant_stoich/product_stoich
			rxn_index = 0
			for j in idRs:
				rxn_index += 1
				met_index = 0
				...
				if i == j:
					...
					for k in idSp:
						met_index += 1
						if k in rxns[i]['products']:
							met_index_0.append(met_index - 1)
							rxns[i]['products'][k] = new_stoich
							S[met_index_change, rxn_index_change] = new_stoich
			unbalanced_rxns_removed.append(i)
	#Identify unbalanced rxns
	metabolite_dict_rxns = {}
	metabolite_dict_rxns_original = {}
